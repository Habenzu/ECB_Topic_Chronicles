...
class Preprocessor: 
    def __init__(self, metadata_df:pd.DataFrame = None, language: str = "english", spacy_model: str = "en_core_web_sm"):
        self.language = language
        self.spacy_nlp = spacy.load(spacy_model, disable=["ner", "parser"])
        self.metadata_df = Metadata().metadata_df if metadata_df is None else metadata_df
    ...
    def tokenize(self, text: str) -> List[str]: 
        """
        gensim.utils.tokenize(text, lower=True) removes numbers, punctuations, special charactersand lower cases it. 
        """
    ...
    def remove_authors(self, text:str) -> str:
        authors = list(set([y for x in self.metadata_df['who'].str.split(',').to_list() for y in x]))
        cleaned_authors = []
        for name in authors: 
            if ' and ' in name:
